...
export function CTAForm({ locale }: { locale: Locale }) {
  ...
  const update = (patch: Partial<typeof cta>) => setContent(locale, { cta: { ...cta, ...patch } })
  return (
    <div className="space-y-3">
...
export function ContactForm({ locale }: { locale: Locale }) {
  ...
  const update = (patch: Partial<typeof contact>) => {
    setContent(locale, { contact: { ...contact, ...patch } })
  }
...
// App Icons Block Form
export function AppIconsForm({ locale }: { locale: Locale }) {
  const { content, setContent } = useCMS()
  const appIcons = content[locale].appIcons || []
  
  const add = () => {
    const copy = appIcons.slice()
    copy.push({ 
      name: locale === "ar" ? "تطبيق جديد" : "New App", 
      src: "/generic-app-logo.png",
      url: "",
      description: locale === "ar" ? "وصف التطبيق" : "App description"
    })
    setContent(locale, { appIcons: copy })
  }
  ...
  const update = (i: number, key: "name" | "src" | "url" | "description", v: string) => {
    const copy = appIcons.slice()
    copy[i] = { ...copy[i], [key]: v }
    setContent(locale, { appIcons: copy })
  }
        ...
        <div key={i} className="rounded-lg border p-3">
          <div className="grid grid-cols-2 gap-2 mb-3">
            <div className="flex items-center gap-2">
              <ImageIcon className="h-4 w-4 text-muted-foreground" />
              <div className="h-8 w-8 overflow-hidden rounded border bg-muted">
                <img src={app.src || "/placeholder.svg"} alt={app.name} className="h-full w-full object-contain" />
              </div>
            </div>
            <Input 
              value={app.name} 
              onChange={(e) => update(i, "name", e.target.value)} 
              placeholder={locale === "ar" ? "اسم التطبيق" : "App name"} 
            />
          </div>
          <div className="grid grid-cols-2 gap-2 mb-3">
            <Input 
          ...
          <Textarea 
            ...
            placeholder={locale === "ar" ? "وصف التطبيق" : "App description"} 
            className="mb-3"
          />
...
// SEO Form
export function SEOForm({ locale }: { locale: Locale }) {
  const { content, setContent } = useCMS()
  ...
  const update = (patch: Partial<typeof seo>) => {
    setContent(locale, { seo: { ...seo, ...patch } })
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <Textarea 
        ...
        placeholder={locale === "ar" ? "وصف الصفحة" : "Page description"} 
      />
      <Input 
        value={seo.keywords} 
        onChange={(e) => update({ keywords: e.target.value })} 
        placeholder={locale === "ar" ? "الكلمات المفتاحية" : "Keywords (comma separated)"} 
      />
      <Input 
        value={seo.ogImage || ""} 
        onChange={(e) => update({ ogImage: e.target.value })} 
        placeholder={locale === "ar" ? "صورة المشاركة" : "Open Graph image URL"} 
      />
      <Input 
        value={seo.canonical || ""} 
        onChange={(e) => update({ canonical: e.target.value })} 
        placeholder={locale === "ar" ? "الرابط الأساسي" : "Canonical URL"} 
      />
    </div>
  )
}
...
// Footer Form
export function FooterForm({ locale }: { locale: Locale }) {
  const { content, setContent } = useCMS()
  ...
  const update = (patch: Partial<typeof footer>) => {
    setContent(locale, { footer: { ...footer, ...patch } })
  }
  
  const addLink = () => {
    const copy = footer.links.slice()
    copy.push({ 
      text: locale === "ar" ? "رابط جديد" : "New link", 
      url: "#" 
    })
    update({ links: copy })
  }
  ...
  const updateLink = (i: number, key: "text" | "url", v: string) => {
    const copy = footer.links.slice()
    copy[i] = { ...copy[i], [key]: v }
    update({ links: copy })
  }
  ...
  const addSocial = () => {
    const copy = footer.social.slice()
    copy.push({ 
      platform: locale === "ar" ? "منصة جديدة" : "New platform", 
      url: "#",
      icon: "🌐"
    })
    update({ social: copy })
  ...
  const updateSocial = (i: number, key: "platform" | "url" | "icon", v: string) => {
    const copy = footer.social.slice()
    copy[i] = { ...copy[i], [key]: v }
    update({ social: copy })
  }
  ...
  return (
    <div className="space-y-4">
      <Input 
        ...
        placeholder={locale === "ar" ? "حقوق النشر" : "Copyright text"} 
      />
      
      <div className="rounded-lg border p-3">
        <div className="mb-2 text-sm font-medium">{locale === "ar" ? "روابط التذييل" : "Footer Links"}</div>
          ...
          {footer.links.map((link, i) => (
            <div key={i} className="flex items-center gap-2">
              <Input 
                ...
                placeholder={locale === "ar" ? "نص الرابط" : "Link text"} 
              />
              <Input 
                value={link.url} 
                ...
                placeholder="URL" 
              />
              <Button variant="destructive" size="icon" onClick={() => removeLink(i)} aria-label="Remove">
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          ))}
        </div>
        <Button size="sm" className="mt-2" onClick={addLink}>
          <Plus className="mr-2 h-4 w-4" />
      ...
      <div className="rounded-lg border p-3">
        <div className="mb-2 text-sm font-medium">{locale === "ar" ? "وسائل التواصل الاجتماعي" : "Social Media"}</div>
        <div className="space-y-2">
          {footer.social.map((social, i) => (
            <div key={i} className="flex items-center gap-2">
              <Input 
                ...
                placeholder={locale === "ar" ? "المنصة" : "Platform"} 
              />
              <Input 
                value={social.url} 
              ...
              <Input 
                ...
                className="w-20"
              />
              <Button variant="destructive" size="icon" onClick={() => removeSocial(i)} aria-label="Remove">
                <Trash2 className="h-4 w-4" />
              </Button>
            </div>
          ))}
        </div>
        <Button size="sm" className="mt-2" onClick={addSocial}>
          <Plus className="mr-2 h-4 w-4" />
          {locale === "ar" ? "إضافة منصة" : "Add Platform"}
        </Button>
      </div>
    </div>
  )
...
// Analytics Form
export function AnalyticsForm({ locale }: { locale: Locale }) {
  const { content, setContent } = useCMS()
  const analytics = content[locale].analytics || { 
    googleAnalytics: "", 
    googleTagManager: "",
    facebookPixel: "",
    hotjar: ""
  ...
  const update = (patch: Partial<typeof analytics>) => {
    setContent(locale, { analytics: { ...analytics, ...patch } })
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <Input 
        ...
        onChange={(e) => update({ googleTagManager: e.target.value })} 
      ...
      />
      <Input 
        value={analytics.hotjar} 
        onChange={(e) => update({ hotjar: e.target.value })} 
        placeholder="Hotjar Site ID" 
      />
      <div className="text-xs text-muted-foreground">
        {locale === "ar" 
          ? "أدخل معرفات التتبع الخاصة بك لمراقبة أداء الموقع" 
          : "Enter your tracking IDs to monitor website performance"
        }
      </div>
    </div>
  )
